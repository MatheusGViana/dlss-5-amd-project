...
ID3D12Resource* Backend::Record(ID3D12GraphicsCommandList* cmd, const Frame& f, const Settings& cfg)
{
    ...
    try
    {
        ...
        const bool settingsChanged = cfg.modelScale != p->lastSettings.modelScale || !p->haveSettings || cfg.tone != p->lastSettings.tone ||
        ...
        for (UINT i = 0; i < p->activePasses; ++i)
        {
            ...
            At<UINT>(r,0x76e44)=0; // Isolated experiment: enable native tone channels
