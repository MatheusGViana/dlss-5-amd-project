...
struct Backend::Impl
{
    ...
    void InitShader()
    {
        ...
        D3D12_DESCRIPTOR_HEAP_DESC hd { D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV, 10,
...
ID3D12Resource* Backend::Record(ID3D12GraphicsCommandList* cmd, const Frame& f, const Settings& cfg)
{
    ...
    {
        if (++p->pendingSkips)
            p->Log("AMD skipped: previous command list awaiting submission; count=" + std::to_string(p->pendingSkips));
        return nullptr;
    }
    ...
    try
    {
        ...
        const bool convertDepth = false;
        const bool resampleMotion = f.motionWidth && f.motionHeight &&
                                    (f.motionWidth != w || f.motionHeight != h);
        ...
        const bool guideChange = p->lastMotionWidth != f.motionWidth || p->lastMotionHeight != f.motionHeight ||
        ...
        auto guideDescriptors = [&](UINT slot, ID3D12Resource* source, ID3D12Resource* target, DXGI_FORMAT format)
        {
            ...
            auto guideSrv = srv;
            guideSrv.Format = source->GetDesc().Format;
        ...
        if (resampleMotion)
        {
            ...
            UINT motionDims[] { w, h, f.motionWidth, f.motionHeight };
        ...
        for (UINT i = 0; i < p->activePasses; ++i)
        {
            ...
            packet.scaleX = f.motionScaleX * (resampleMotion ? float(w) / f.motionWidth : 1.0f);
            packet.scaleY = f.motionScaleY * (resampleMotion ? float(h) / f.motionHeight : 1.0f);
        ...
        if (cfg.rtgi.enabled && !p->rtgiFailed)
        {
            try
            {
                ...
                rtgiFrame.motionScaleX *= resampleMotion ? float(w) / f.motionWidth : 1.0f;
                rtgiFrame.motionScaleY *= resampleMotion ? float(h) / f.motionHeight : 1.0f;
