...
int wmain(int argc, wchar_t **argv) {
  try {
    ...
    AmdPreSr::ColorEncoding decode(device.Get()),encode(device.Get());
    size_t badFrames = 0;
    for (int iteration = 0; iteration < 8; ++iteration) {
      ...
      auto linear=decode.Run(cmd.Get(),frame.colour,frame.colourState,active,active,2+iteration%2,false);
      auto out=encode.Run(cmd.Get(),linear,D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE,active,active,2+iteration%2,true);
      if (!out && forceTimeout) {
      ...
      for (UINT y = 0; y < active; ++y)
        for (UINT x = 0; x < active; ++x) {
          ...
          const uint16_t expected[4]={uint16_t(0x3000+(x%128)*12),uint16_t(0x3400+(y%128)*8),0x3800,0x3c00};
          for(int c=0;c<3;++c) if(std::abs(halfValue(p[c])-halfValue(expected[c]))>.001f) ++changed;
          if(p[3]!=expected[3])++changed;
          for (int c = 0; c < 3; ++c)
      ...
      const bool expectUnchanged = true;
